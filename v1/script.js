...
const calcDisplaySummary = function (acc) {
  ...
  labelSumInterest.textContent = `${Math.abs(interest).toFixed(4)} €`;
};
...
btnSort.addEventListener('click', function (e) {
  e.preventDefault();
  displayMovements(currentAcc.movements, !sor);
  sor = !sor;
});
...
if (sarahDog.curFood > sarahDog.recommendedFood) {
  console.log(`${sarahDog.owners[0]}'s Dog is Eating too Little`);
} else if (sarahDog.curFood < sarahDog.recommendedFood) {
  console.log(`${sarahDog.owners[0]}'s Dog is Eating too Much`);
}
// 3.
...
// };
// console.log(strToTitleCase('this is'));
// console.log(strToTitleCase('this is a LONG title'));
// console.log(strToTitleCase('and here is another title with an EXAMPLE'));
// const movementsUsd = movements.map(mo => mo * euroToUsd);
...
// for (const move of movements) move>0 ? console.log(`You deposited ${move}`): console.log(`You withdrew ${Math.abs(move)}`);
...
// 0: function(200)
// 1: function(450) ,,,,,,,,,,,,,,,,, n-1: function(1300)
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  // slice removes first element & last two elements
  const newDogsArr = correctedDogsJulia.concat(dogsKate);
  ...
  // console.log(correctedDogsJulia, dogsKate);
...
const total = movements.reduce(((accumulator, cur) => accumulator + cur),0);
console.log(total);
// maximum value
...
const calcAverageHumanAge = function (ages) {
  ...
  let sumAd = 0;
  for (const ad of adultDogs) sumAd = sumAd + ad;
  const avgAd = sumAd / adultDogs.length;
  console.log(humanAges, avgAd);
};
...
console.log(account);
for (const acc of accounts) console.log(acc.owner);
console.log(movements);
// Equality
